<template>
    <div>
      <div class="cheta-m10-30">
        <a-input class="cheta-input-text" v-model="detailObject.name" @change="detailObject.changeInput = true" @blur="changeInput"/>
      </div>
      <div class="cheta-mlr-44">
          ...
          <a-col :span="16">
            <a-row>
              ...
              <a-col :span="6">
                <div class="cheta-event">
                  <a-dropdown :trigger="['click']">
                    <span @click="priorityVisible = true">
                      <div class="cheta-box-flex">
        ...
        <a-row :gutter="16" class="cheta-row cheta-font16 cheta-project-total">
          ...
          <a-col :span="8" :xs="24" :sm="12" :md="8" :lg="8" :xl="8" class="cheta-project">
            <div><div class="cheta-lable">关联项目</div>
              <router-link target="_blank" :to="{ path: `/firm/project/detail/${projectWork.relId}/renwufenpei` }">{{projectWork.relName}}</router-link>
            </div>
        ...
        <div class="cheta-row">
          ...
          <div v-if="!detailObject.description && !focus">
            <a-textarea :rows="4" placeholder="添加备注" @focus="handlefocus" :class="`cheta-textarea ${focus? 'cheta-textarea-none cheta-edit-text' : ''}`"></a-textarea>
          </div>
          <quill-editor
            v-else
            ref="myQuillEditor"
            ...
            @focus="onEditorFocus"
            placeholder="选择项目关联"
            v-model="detailObject.description"
            :class="`cheta-edit-text ${focus ? 'cheta-edit-text-focus' : ''}`"
          >
          </quill-editor>
        </div>
        <!--工时-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeInput () {
        if (this.detailObject.changeInput) {
          this.changeData({name: this.detailObject.name})
        }
      },
      changeData (param) {
...
<style lang="less">
  ...
  .cheta-edit-text .ql-toolbar.ql-snow + .ql-container.ql-snow {
    border-top: 1px solid #ccc;
  }
  .cheta-edit-text .ql-toolbar.ql-snow{display: none;}
  .cheta-edit-text-focus{
    height: 200px!important;
    margin-bottom: 60px;
    .ql-toolbar.ql-snow{display: block;}
