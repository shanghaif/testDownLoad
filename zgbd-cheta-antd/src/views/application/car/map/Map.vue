...
<script>
  ...
  import { getLocation } from '../api/util.js';
  import { areaMark, streetMark, carMark,carInfo } from '../api/tpl.js';
  var thisMap = {
    map: null,
    marker: [],
    infoWindow: null
  ...
  export default {
    ...
    methods: {
      ...
      addCar(list){
        ...
        list.forEach(e => {
          // debugger
          let status = constant[e.statusRun];
          if (!status) {
      ...
      getMapByZoom(markList, zoom) {
        ...
        getAction('/application/car/gps/get',param).then(res => {
          ...
          if (param.grade === 5) {
            $this.$emit('listShow')
            $this.$emit('changeList', list, $this.clickTitle);
          }
