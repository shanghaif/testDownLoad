    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          :label="menuLabel"
          hasFeedback >
          <a-input placeholder="请输入页面/分类名称" v-decorator="[ 'name', validatorRules.name]" :readOnly="disableSubmit"/>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          name:{rules: [{ required: true, message: '请输入页面/分类名称!' }]},
        },
        ...
        url: {
          ...
          listPmpage: "/application/setting/mpage/getPTreeData",
          listPFmpage: "/application/setting/mpage/getFtreeData",
        }
    ...
    methods: {
      ...
      initialMpageList(){
        ...
        getAction(this.url.listMpage,params).then((res)=>{
          console.log(this.url.listMpage)
          console.log(res)
          if(res.success){
      ...
      initialPmenuList(){
        var params = "";
        getAction(this.url.listPmpage,params).then((res)=>{
          console.log(this.url.listPmpage)
      ...
      initialPFmenuList(){
        var params = "";
        getAction(this.url.listPFmpage,params).then((res)=>{
          console.log(this.url.listPFmpage)
          console.log(res.result)
          if(res.success){
            this.fTreeData = res.result;
          }else{
            console.log(res.message);
          }
        });
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let obj;
            if(!this.model.id){
              obj = postAction("/application/setting/mpage/saveMpage", {mpage:{name:formData.name,parentId: formData.parentId,url: formData.url}});
            }else{
              obj = postAction("/application/setting/mpage/editMpage", {mpage:{id: formData.id,name:formData.name,parentId: formData.parentId,url: formData.url}});
            }
            obj.then((res)=>{
              if(res.success){
                console.log(res)
