...
<script>
  ...
  import {reportSystemData} from '../../firm/report/api/api'
  ...
  export default {
    ...
    created() {
      /*this.getData()*/
      this.init({ menuId: 'b267cb32be5d4d04a68f7e5d72993d9a' });
    }
    ,
    methods: {
      init(params) {
        var that = this;
        getAction('/application/firm/report/getAutoReport', params).then(res => {
      ...
      addReport() {
        // this.$emit('addReport')
        debugger
        this.$router.push({ path: '/xinjianliebiao' })
      },
      // getDetail (item) {
      ...
      getDetail(item) {
        debugger
        // this.$router.push({path:`/xinzengbaobiao/${this.item}`})
        this.$router.push({name: '报表信息', params: item})
      },
    }
  }
